...
class CiteGuardEvaluator:
    """
    Evaluates the CiteGuard pipeline against a golden-standard dataset.

    CRITICAL: accepts an existing Orchestrator instance to REUSE already-loaded
    models. Do NOT instantiate a new Orchestrator here — that doubles RAM usage
    by loading CrossEncoder + NLI a second time.
    """

    def __init__(self, dataset_path: str = "evaluation/golden_standard.json", orchestrator: Optional[Orchestrator] = None):
        self.dataset_path = dataset_path
        # Reuse the orchestrator from routes.py — do not create a new one
    ...
    async def evaluate(
        ...
        for item in dataset:
            ...
            try:
                ...
                for gold_claim in item.get("claims", []):
                    total_claims += 1
                    ...
                    if best_match:
                        ...
                        gold_evidence = gold_claim.get("gold_evidence", "")
                        found_rank = -1
                        for rank, ev in enumerate(best_match.evidence_list, start=1):
                            if (
                                self._token_overlap(gold_evidence, ev.evidence_text) > 0.35
                                or self._token_overlap(ev.evidence_text, gold_evidence) > 0.35
                            ):
                                found_rank = rank
                                break
                ...
                logger.error(f"Failed to process {item.get('document_path', '?')}: {e}")
